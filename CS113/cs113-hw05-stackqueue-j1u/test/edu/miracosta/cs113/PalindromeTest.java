...
public class PalindromeTest {
    ...
    private boolean isPalindrome(String s) {
        if(s != null){
            ...
            ArrayListStack firstStack = new ArrayListStack();
            ArrayListStack secondStack = new ArrayListStack();

            pushToStacks(firstStack, secondStack, s);
    ...
    /**
     * Helper method which pushes the objects into the array
     * @param firstStack first stack which the entire string is initially pushed in to
     * @param secondStack second stack which half of the first stack is pushed into
     * @param s string to be checked ~must be in lowercase and without spaces(handled in isPalindrome())~
     */
    private void pushToStacks(ArrayListStack firstStack, ArrayListStack secondStack, String s) {

        int numberToPush = s.length() / 2;
        ...
        if ((s.length() % 2) != 0) { //odd
            for (int i = 0; i <= numberToPush; i++) {
                if (i == numberToPush) {
                    firstStack.pop(); //removes the hanging letter in an odd palindrome
                    return;
                }
