/**
 * edu.miracosta.cs113.Polynomial.java : JUnit test for edu.miracosta.cs113.Polynomial ADT implementation.
 ...
 *
 * Assuming that edu.miracosta.cs113.Term works and has no errors (make sure all TermTest tests pass!),
 * testing relies on building edu.miracosta.cs113.Term constructors, equals(), etc.
 ...
 * as a guide to help you program edu.miracosta.cs113.Polynomial and account for all situations. You can
...
public class PolynomialTest {
    ...
    public void testUniqueAddTermTypeBySizeOfList1() {
        ...
        for(int i = 0; i < UNIQUE_TERMS1.length; i++) {
            ...
            assertEquals("edu.miracosta.cs113.Term was NOT added to list of terms", i+1, test.getNumTerms());
        }
    }
    ...
    public void testDuplicateAddTermTypeBySizeOfList() {
        ...
        for(int i = finalSize; i < DUPLICATE_TERMS.length; i++) {
            test.addTerm(DUPLICATE_TERMS[i]);
            assertEquals("edu.miracosta.cs113.Term was added but DIDN'T combine like terms (matching exponents",
    ...
    public void testCopyConstructor() {
        ...
        assertTrue("Shallow copy of edu.miracosta.cs113.Polynomial object", original != copy);

        for(int i = 0; i < original.getNumTerms(); i++) {
            originalTerm = original.getTerm(i);
            copyTerm = copy.getTerm(i);

            assertTrue("Shallow copy of edu.miracosta.cs113.Term object in edu.miracosta.cs113.Polynomial", originalTerm != copyTerm);
     ...
     * add tests for:
     * - duplicate add terms by string (not edu.miracosta.cs113.Term objects),
